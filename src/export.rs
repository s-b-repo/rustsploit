...
pub async fn export_csv_string() -> Result<String> {
    ...
    for s in &data.services {
        output.push_str(&format!(
            ...
            csv_escape(&s.host),
            s.port,
            csv_escape(&s.protocol),
            csv_escape(&s.service_name),
            csv_escape(s.version.as_deref().unwrap_or(""))
        ));
    }
    output.push('\n');
    ...
    for l in &data.loot {
        output.push_str(&format!(
            ...
            csv_escape(&l.id),
            csv_escape(&l.host),
            csv_escape(&l.loot_type),
            csv_escape(&l.description),
            csv_escape(&l.filename),
...
pub async fn export_summary(path: &str) -> Result<()> {
    ...
    crate::mprintln!(
        "{}",
        format!("[+] Exported summary report to '{}'", path).green()
    );
    Ok(())
}
...
pub fn validate_export_path(path: &str) -> Result<()> {
    if path.is_empty() || path.len() > 255 {
        return Err(anyhow::anyhow!(
            "Invalid export path length (max 255 chars)"
        ));
    }
    ...
    {
        return Err(anyhow::anyhow!(
            "Only filenames are allowed for export (no directory separators). Use a relative filename like 'report.json'."
        ));
    }
    if path.starts_with('.') {
