...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    match client.get(&url).send().await {
        ...
        Err(e) => tracing::warn!("wp-json users probe failed on {}: {}", url, e),
    }
    ...
    for n in 1..=max_author_id {
        ...
        match redir_client.get(&url).send().await {
            Ok(r) => {
                if r.status().is_redirection() {
                    let loc = match r.headers().get("location") {
                        Some(v) => match v.to_str() {
                            Ok(l) => l,
                            Err(e) => {
                                tracing::warn!("?author={} Location header is non-ASCII: {}", n, e);
                                continue;
                            }
                        },
                        None => {
                            tracing::warn!("?author={} redirect has no Location header", n);
                            continue;
                        }
                    };
                    let slug = loc
                    ...
                    if let Some(slug) = slug {
                        ...
                        outcome.findings.push(Finding {
                            ...
                            kind: FindingKind::Note,
                            message: format!(
                                "WP user slug leak: ?author={} -> /author/{}",
                                n, slug
                            ),
                            data: Some(serde_json::json!({
            ...
            Err(e) => tracing::warn!("?author={} probe failed on {}: {}", n, url, e),
        }
    }
    ...
    match client.get(&oembed_url).send().await {
        ...
        Err(e) => tracing::warn!("oembed author probe failed on {}: {}", oembed_url, e),
    }
