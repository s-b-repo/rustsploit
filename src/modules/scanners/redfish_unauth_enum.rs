...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    for s in schemes {
        ...
        match client.get(&root).send().await {
            ...
            Ok(r) => {
                last_probe_error = Some(format!("HTTP {}", r.status()));
                if verbose {
                    crate::mprintln!("{}", format!("[-] {} -> {}", root, r.status()).dimmed());
                }
            }
            Err(e) => {
                last_probe_error = Some(format!("{e}"));
                if verbose {
    ...
    let Some(scheme) = working_scheme else {
        ...
        // Distinguish "unreachable (transport/HTTP error)" from a confirmed
        // scan with no leaks: push a Note carrying the last probe error so
        // automation can tell a connectivity failure apart from a safe host.
        let last_err_desc = match last_probe_error {
            Some(ref e) => e.clone(),
        ...
        outcome.findings.push(Finding {
            ...
            message: format!(
                "Redfish endpoint unreachable on {} (last probe: {}); host could not be assessed",
                host, last_err_desc
            ),
            data: Some(serde_json::json!({
                "host": host,
                "status": "unreachable",
                "last_probe_error": last_err_desc,
            })),
        });
        return Ok(outcome);
    };
            ...
            match client.get(&url).send().await {
                Ok(resp) => {
                    ...
                    if status.is_success() {
                        ...
                        let len = body.len();
                        // Content-validate before flagging: a bare 200 can be a
                        // login redirect rendered as 200, a marketing page, or an
                        // empty body. Each aux endpoint needs its expected marker
                        // plus a minimum body size; anything else is only a Note
                        // pending manual validation.
                        let lower = body.to_lowercase();
                        let marker_hit = match *path {
                            "/cc/bmc_cc.xml" => {
                                len >= 64
                                    && (lower.contains("<?xml")
                                        || lower.contains("<bmc")
                            ...
                            "/api/settings/lan" => {
                                len >= 32 && (lower.contains("lan") || lower.contains('{'))
                            }
                            "/api/system/system_info" => {
                                len >= 32 && (lower.contains("system") || lower.contains('{'))
                            }
                            _ => false,
                        };
                        if marker_hit {
                            ...
                            outcome.findings.push(Finding {
                                ...
                                kind: FindingKind::Vulnerable,
                                message: format!(
                                    "Auxiliary unauth endpoint {} ({}) returned {} bytes with expected content marker",
                                    url, label, len
                                ),
                                data: Some(serde_json::json!({
                                    "host": host,
                            ...
                            crate::mprintln!(
                                "{}",
                                format!(
                                    "[~] {} ({}) -> 200 ({} bytes) but content marker not matched — unconfirmed",
                                    path, label, len
                                )
                                .yellow()
                            );
                            outcome.findings.push(Finding {
                                ...
                                kind: FindingKind::Note,
                                message: format!(
                                    "Auxiliary endpoint {} ({}) returned {} bytes but content did not match the expected marker — leak unconfirmed",
                                    url, label, len
                                ),
                                data: Some(serde_json::json!({
                                    "host": host,
                ...
                Err(e) => {
                    tracing::warn!("aux endpoint {} probe failed: {e}", url);
                    if verbose {
                        crate::mprintln!("{}", format!("[-] {}: {}", path, e).dimmed());
                    }
                }
