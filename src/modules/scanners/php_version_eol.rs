...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    for scheme in ["https", "http"] {
        ...
        match client.get(&url).send().await {
            Ok(resp) => {
                let powered_by = match resp.headers().get("x-powered-by") {
                    Some(v) => match v.to_str() {
                        Ok(s) => Some(s.to_string()),
                        Err(e) => {
                            tracing::debug!("X-Powered-By header not UTF-8: {e}");
                            None
                        }
                    },
                    None => None,
                };
                if let Some(banner) = powered_by {
                    php_seen = Some((url.clone(), banner.clone()));
            ...
            for scheme in ["http", "https"] {
                ...
                let resp = match client.get(&url).send().await {
                    ...
                    Err(e) => {
                        crate::mprintln!(
                            "{}",
                            format!("[-] {} probe failed: {}", url, redact_err(&e.to_string()))
                                .dimmed()
                        );
                        continue;
                    }
                ...
                let banner = match resp.headers().get("x-powered-by") {
                    Some(v) => match v.to_str() {
                        Ok(s) => s.to_string(),
                        Err(e) => {
                            tracing::debug!("X-Powered-By header not UTF-8: {e}");
