...
async fn setup_wizard(initial_target: &str) -> Result<DirBruteConfig> {
    ...
    // 4. Performance
    let concurrency_raw = cfg_prompt_default("concurrency", "Concurrency (Threads)", "10").await?;
    let concurrency: usize = match concurrency_raw.trim().parse::<usize>() {
        Ok(v) => v,
        Err(e) => {
            crate::meprintln!(
                "[!] Invalid concurrency '{}' ({}); using default 10",
                concurrency_raw,
                e
            );
            10
    ...
    let delay_raw = cfg_prompt_default("delay_ms", "Delay per request (ms)", "200").await?;
    let delay_ms: u64 = match delay_raw.trim().parse::<u64>() {
        Ok(v) => v,
        Err(e) => {
            crate::meprintln!(
                "[!] Invalid delay '{}' ({}); using default 200ms",
                delay_raw,
                e
            );
            200
...
async fn execute_scan(ctx: &ModuleCtx, config: DirBruteConfig) -> Result<ModuleOutcome> {
    ...
    let client = crate::utils::network::build_http_client_with(
        ...
        crate::utils::network::HttpClientOpts {
            ...
            accept_invalid_certs: !crate::utils::network::get_global_strict_tls(),
            ..Default::default()
        },
    )
    ...
    // Stream the wordlist through a bounded worker pool: only `concurrency`
    // futures are alive at any moment. (The previous implementation spawned
    // one tokio task per wordlist line up-front, allocating millions of
    // futures/JoinHandles for large wordlists regardless of the concurrency
    // setting.)
    stream::iter(lines.iter().cloned())
        .map(|word| {
            ...
            async move {
                ...
                for method in &methods {
                    // Stop destructive methods promptly on cancellation —
                    // after Ctrl+C no further DELETE/PUT/POST requests may
                    // be issued for this word.
                    if cancel.is_cancelled() || crate::context::is_cancelled() {
                        return;
                    }
