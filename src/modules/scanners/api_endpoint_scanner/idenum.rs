...
pub(super) async fn perform_id_enumeration(
    ...
    for payload in payloads {
        ...
        match req_builder.send().await {
            Ok(resp) => {
                ...
                if status != reqwest::StatusCode::NOT_FOUND {
                    ...
                    if status.is_success() {
                        ...
                        match File::create(&body_file) {
                            Ok(mut f) => {
                                if let Err(e) =
                                    crate::utils::set_secure_permissions(&body_file, 0o600)
                                {
                                    crate::meprintln!(
            ...
            Err(e) => {
                // Transport failure for this ID — log it to the results file and
                // the console instead of silently treating it as "no hit".
                crate::meprintln!("[!] Request failed for {}: {}", label, e);
                if let Err(werr) = writeln!(
                    results_file_handle,
                    "[{}] {} - Request failed: {}",
                    ...
                    crate::meprintln!("[!] Logging failed: {}", werr);
                }
            }
        }
    }
