    ...
    match req_builder.send().await {
        ...
        Err(e) => {
                ...
                .open(spec.result_file)
            {
                ...
                Err(open_err) => {
                    tracing::debug!(
                        "could not open error log {}: {} (request error: {e})",
                        spec.result_file.display(),
                        open_err
                    );
                    crate::meprintln!(
                        "\n{}",
                        format!("[!] Request failed for {}: {}", full_label, e).red()
                    );
                }
            }
        }
...
async fn log_response(
    ...
    match String::from_utf8(body.to_vec()) {
        ...
        Err(e) => {
            ...
            tracing::debug!("response body from {url} is not UTF-8: {e}");
        }
    }
    writeln!(
        file,
