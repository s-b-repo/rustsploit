...
async fn check_user(client: &reqwest::Client, username: &str) -> Result<i64> {
    ...
    .await
    {
        Ok(t) => t,
        Err(e) => {
            return Err(e).context("GetCredentialType response body read failed");
        }
    };
    // A non-JSON body (HTML throttle/interstitial/WAF block) or a JSON body
    // without IfExistsResult must NOT silently collapse to "unknown": that
    // made a sustained 429/WAF condition look like an empty enumeration.
    // Both cases surface as errors so the caller can warn about throttling.
    let v: serde_json::Value = serde_json::from_str(&txt).with_context(|| {
        format!(
            "GetCredentialType returned a non-JSON body ({} bytes): {}",
            txt.len(),
    ...
    let result = v
        .get("IfExistsResult")
        .and_then(|x| x.as_i64())
        .context("GetCredentialType response is missing the IfExistsResult field")?;
    Ok(result)
}
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    // If a large fraction of probes failed (non-JSON bodies / missing fields,
    // typically throttling or a WAF block), the enumeration never actually
    // worked — surface that loudly instead of letting it look like an empty
    // tenant.
    if probes > 0 && probe_failures * 2 >= probes {
        let msg = format!(
            "{}/{} GetCredentialType probes failed (non-JSON/throttled) — enumeration results are UNRELIABLE, back off and retry",
            probe_failures, probes
        );
        crate::meprintln!("{}", format!("[!] {}", msg).yellow());
        outcome.findings.push(Finding {
            target: tenant_label.clone(),
            kind: FindingKind::Note,
            message: format!(
                "M365 user enumeration unreliable for {}: {}",
                tenant_label, msg
            ),
            data: Some(serde_json::json!({
                "tenant": tenant_label,
