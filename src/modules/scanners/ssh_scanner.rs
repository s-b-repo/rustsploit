...
use crate::utils::{cfg_prompt_default, cfg_prompt_output_file, cfg_prompt_yes_no};
use anyhow::Context;
...
fn display_banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   SSH Service Scanner                                             ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Scan networks for SSH services and grab banners                 ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║                                                                   ║".cyan()
    ...
    crate::mprintln!(
        "{}",
        "║   - CIDR range support                                            ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   - IPv4/IPv6 support                                             ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   - Banner grabbing                                               ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   - Concurrent scanning                                           ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   - Results export                                                ║".cyan()
...
impl Statistics {
    ...
    fn print_summary(&self) {
        ...
        crate::mprintln!(
            "SSH services found: {}",
            self.ssh_found.load(Ordering::Relaxed).to_string().green()
        );
            ...
            && let Ok(network) = s.parse::<IpNetwork>()
        {
            ...
            if host_count > MAX_CIDR_HOSTS as u128 {
                crate::mprintln!(
                    "{}",
                    format!(
                        "[!] CIDR {} has {} hosts — scanning first {} only",
                        s, host_count, MAX_CIDR_HOSTS
                    )
                    .yellow()
                );
            }
...
pub async fn scan_ssh(
    ...
    for (host, port) in targets {
        ...
        let handle: tokio::task::JoinHandle<Result<()>> = tokio::spawn(async move {
            ...
            let result =
                spawn_blocking(move || grab_ssh_banner(&host_clone, port, timeout_secs)).await;
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    // Get scan options
    let threads_str = cfg_prompt_default(
        "threads",
        "Concurrent threads",
    ...
    let timeout_str = cfg_prompt_default(
        "timeout",
    ...
    {
        let output_path =
            cfg_prompt_output_file("output_file", "Output file", "ssh_scan_results.txt").await?;
    ...
    crate::mprintln!(
        "{}",
        format!(
            "[*] SSH scanner complete. Found {} services.",
            results.len()
        )
        .green()
    );
