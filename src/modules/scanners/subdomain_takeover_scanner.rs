...
const FINGERPRINTS: &[(&str, &str, &str)] = &[
    ...
    (
        "WPEngine",
        ".wpengine.com",
        "The site you were looking for couldn",
    ),
    (
        "Beanstalk",
...
fn banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   Subdomain Takeover Detector                                ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Resolves CNAME → matches provider → checks live fingerprint║".cyan()
    );
    crate::mprintln!(
        "{}",
        "╚══════════════════════════════════════════════════════════════╝".cyan()
...
async fn dns_lookup(name: &str, rtype: RecordType, resolver: &str) -> Result<Vec<String>> {
    ...
    let bg_handle = tokio::spawn(async {
    ...
    .await
    {
        Ok(Ok(r)) => r,
        Ok(Err(e)) => {
            // Stop the per-lookup connection driver (and its UDP socket) on
            // every exit path — a detached task would otherwise leak one task +
            // socket per CNAME hop per host under a CIDR fan-out.
            bg_handle.abort();
            return Err(anyhow::anyhow!("DNS query for {} failed: {}", name, e));
        }
        Err(e) => {
            bg_handle.abort();
            return Err(anyhow::anyhow!("DNS query for {} timed out: {}", name, e));
        }
    };
    bg_handle.abort();
...
async fn resolve_cname_chain(host: &str, resolver: &str) -> Vec<String> {
    ...
    for _ in 0..10 {
        match dns_lookup(&current, RecordType::CNAME, resolver).await {
            ...
            Err(e) => {
                tracing::debug!("CNAME lookup failed: {e}");
                break;
            }
        }
    }
...
async fn resolves_to_a(host: &str, resolver: &str) -> bool {
    dns_lookup(host, RecordType::A, resolver)
        .await
        .map(|v| !v.is_empty())
        .unwrap_or(false)
}
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let host =
        cfg_prompt_default("host", "Target host (no scheme)", &sanitize_host(target)).await?;
    let resolver = cfg_prompt_default("resolver", "DNS resolver", "1.1.1.1").await?;

    crate::mprintln!(
        "{}",
        format!("[*] Resolving CNAME chain for {} via {}...", host, resolver).cyan()
    );
    ...
    if let Some(p) = matched_provider {
        crate::mprintln!(
            "{}",
            format!(
                "[!] Provider matched: {} (terminal CNAME = {})",
                p, terminal
            )
            .yellow()
        );
        ...
        crate::mprintln!(
            "{}",
            "[~] Terminal CNAME does not match any known takeover provider.".dimmed()
        );
    }
    ...
    for scheme in ["https", "http"] {
        ...
        match client.get(&url).send().await {
            Ok(r) => {
                ...
                crate::mprintln!(
                    "{}",
                    format!("[*] {} status={} body[0..200]={:?}", url, status, snippet).cyan()
                );
                    ...
                    && body.contains(fp)
                {
                    ...
                    outcome.findings.push(Finding {
                        ...
                        kind: FindingKind::Vulnerable,
                        message: format!(
                            "subdomain takeover candidate: {host} matches {} fingerprint at {url}",
                            matched_provider.unwrap_or("?")
                        ),
                        data: None,
                    });
                }
