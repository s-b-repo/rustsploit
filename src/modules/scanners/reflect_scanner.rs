...
fn check_dns(buf: &[u8], n: usize, probe_len: usize) -> Option<ProbeResult> {
    ...
    // Byte 2: QR(1) Opcode(4) AA(1) TC(1) RD(1); byte 3: RA(1) Z(3) RCODE(4).
    let qr = (buf[2] >> 7) & 1;
    // Recursion Available — authoritative-only nameservers answer our query
    // (QR=1) but never offer recursion, so they are NOT usable amplification
    // resolvers even though they reply.
    let ra = (buf[3] >> 7) & 1;
    ...
    if rcode == 5 {
        return None; // REFUSED = not open
    }
    // Only flag a resolver when it (a) advertises recursion AND (b) actually
    // answered the query for a third-party domain with records. A SERVFAIL or
    // an empty answer section means the server did not perform recursion for
    // us — typical of authoritative-only servers — and is not an amplifier.
    if ra != 1 {
        tracing::debug!("DNS reply with RA=0 (authoritative-only) — not an open resolver");
        return None;
    }
    if rcode != 0 || answer_count == 0 {
        tracing::debug!(
            "DNS reply rcode={} answers={} — no recursion performed, not an open resolver",
            rcode_str,
            answer_count
        );
        return None;
...
async fn probe_single(
    ...
    let sock = match crate::utils::udp_bind(None).await {
        Ok(s) => s,
        Err(e) => {
            tracing::warn!("UDP bind failed for {}:{} probe: {}", ip, port, e);
            return None;
        }
    };
    let addr = SocketAddr::new(ip, port);
    if let Err(e) = sock.send_to(payload, addr).await {
        tracing::warn!("UDP probe send to {} failed: {}", addr, e);
        return None;
    }
    ...
    match timeout(dur, sock.recv_from(&mut buf)).await {
        Ok(Ok((n, _))) => {
            if n == 0 {
                tracing::debug!("empty datagram from {}:{} (0 bytes)", ip, port);
                return None;
            }
            check(&buf, n, payload.len())
        }
        Ok(Err(e)) => {
            tracing::debug!("probe recv error from {}:{}: {}", ip, port, e);
            None
        }
        Err(e) => {
            tracing::debug!("probe to {}:{} timed out: {}", ip, port, e);
            None
        }
    }
}
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let timeout_ms: u64 = match cfg_prompt_default("timeout", "Probe timeout (ms)", "3000")
        .await?
        .parse()
    {
        Ok(v) => v,
        Err(e) => {
            crate::meprintln!("[!] Invalid probe timeout ({}); falling back to 3000ms", e);
            3000
        }
    };
    ...
    let verbose = if batch {
        ...
        cfg_prompt_yes_no("verbose", "Verbose output?", false).await?
    };
        ...
        Err(e) => {
            ...
            match tokio::net::lookup_host(format!("{}:0", target)).await {
                Ok(mut addrs) => match addrs.next() {
                    Some(sa) => sa.ip(),
                    None => return Err(anyhow!("No addresses resolved for target: {}", target)),
                },
                Err(resolv_err) => {
                    return Err(anyhow!("Cannot resolve target {}: {}", target, resolv_err));
                }
            }
        }
    };
