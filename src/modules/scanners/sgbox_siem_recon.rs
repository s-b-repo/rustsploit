...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    match probe_login_rate_limit(&client, &login_url).await {
        Ok(p) => {
            // "No rate limit" may only be asserted when the server ACCEPTED
            // normal-attempt responses the whole time. 429/503 (with or
            // without Retry-After) and transport drops are evidence that rate
            // limiting IS present (or that the probe could not complete).
            let signal = if p.saw_retry_after {
                "rate-limited (Retry-After observed)"
            } else if p.rate_limited_responses > 0 {
                "rate-limited (429/503 responses observed)"
            } else if p.rate_limited_responses + p.transport_errors >= p.successes {
                "behaviour inconclusive (throttled or connection drops)"
            ...
            .await;
            if !p.saw_retry_after
                && p.rate_limited_responses == 0
                && p.transport_errors == 0
                && p.successes >= 5
            {
                ...
                outcome.findings.push(crate::module::Finding {
                    ...
                    data: Some(serde_json::json!({
                        "host": host,
                        "attempts_accepted": p.successes,
                        "rate_limited_responses": p.rate_limited_responses,
        ...
        Err(e) => {
            crate::meprintln!("[!] rate-limit probe failed: {e:#}");
        }
    }
    ...
    let want_report = if crate::utils::is_batch_mode() {
        ...
        cfg_prompt_yes_no("write_report", "Save markdown report to loot?", true).await?
    };
    if want_report {
...
fn extract_license_owner_from_vars(body: &str) -> Option<String> {
    ...
    let v: serde_json::Value = match serde_json::from_str(&json) {
        Ok(v) => v,
        Err(e) => {
            tracing::debug!("window._vars payload is not valid JSON: {e}");
            return None;
        }
    };
    let owner = v
...
fn decode_window_vars(body: &str) -> Option<String> {
    ...
    let bytes = match base64::engine::general_purpose::STANDARD.decode(b64) {
        Ok(b) => b,
        Err(e) => {
            tracing::debug!("window._vars base64 decode failed: {e}");
            return None;
        }
    };
    match String::from_utf8(bytes) {
        Ok(s) => Some(s),
        Err(e) => {
            tracing::debug!("window._vars payload is not UTF-8: {e}");
            None
        }
    }
}
    ...
    for (code, name) in KNOWN_MODULES {
        ...
        let resp = match client.get(&url).send().await {
            ...
            Err(e) => {
                crate::mprintln!(
                    "{}",
                    format!("[-] {} module probe failed: {}", code, e).dimmed()
                );
                continue;
            }
        };
...
struct LoginRateProbe {
    /// Responses ACCEPTED as normal login traffic (2xx/4xx form rejections).
    successes: u32,
    /// 429/503 responses — exactly what a rate-limiting server returns.
    rate_limited_responses: u32,
    /// Transport failures (connect/timeout/TLS) — probe could not complete.
        ...
        match client.post(login_url).form(&form).send().await {
            Ok(resp) => {
                ...
                if code == 429 || code == 503 {
                    // The server IS throttling us: this must count as evidence
                    // OF rate limiting, never as "attempt accepted".
                    p.rate_limited_responses += 1;
                } else {
                    p.successes += 1;
                }
            }
