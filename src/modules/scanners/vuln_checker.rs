...
pub fn info() -> ModuleInfo {
    ModuleInfo {
        ...
        description:
            "Runs fingerprint probes against a target to identify vulnerable products/services.\n\
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let mut errors = 0u32;
    // Open-port and banner results are collapsed per port instead of emitting
    // one note per CVE probe that shares the port (dozens of low-signal
    // findings for any host with 443/80/5900 open).
    let mut open_ports: std::collections::BTreeSet<u16> = std::collections::BTreeSet::new();
    let mut open_port_modules: std::collections::BTreeMap<u16, Vec<&'static str>> =
        std::collections::BTreeMap::new();
    let mut banner_hits: std::collections::BTreeMap<(u16, &'static str), Vec<&'static str>> =
        std::collections::BTreeMap::new();
    let mut banner_samples: std::collections::BTreeMap<(u16, &'static str), String> =
        std::collections::BTreeMap::new();
    ...
    for (i, probe) in filtered.iter().enumerate() {
        ...
        // Throttle the ~200-probe burst per host through the framework's
        // hierarchical rate limiter (global → per-module → per-target).
        ctx.rate_limit(&target).await;
        let result = run_probe(probe, &target, http_client.as_ref()).await;

        match &result {
            ProbeResult::OpenPort(port) => {
            ...
            ProbeResult::BannerMatch {
                ...
                banner_hits
                    ...
                    .or_default()
                    .push(probe.module);
                // Keep the banner for the (single) aggregated note below.
                banner_samples.insert((*port, *marker), banner.clone());
            }
    ...
    // One aggregated note per bare open port (positive: TCP connect succeeded;
    // explicitly NOT a service/version confirmation).
    if !open_ports.is_empty() {
        let ports_csv: Vec<String> = open_ports.iter().map(|p| p.to_string()).collect();
        let module_count: usize = open_port_modules.values().map(|v| v.len()).sum();
        crate::mprintln!(
            "{} Open TCP port(s) at {}: {} ({} probe(s) matched port-open only)",
            "[?]".yellow(),
            target,
            ports_csv.join(", "),
            module_count
        ...
        crate::events::emit(crate::events::ModuleEvent::ServiceDetected {
            host: target.to_string(),
            port: 0,
            service: "vuln:open-tcp-ports".to_string(),
            version: Some(ports_csv.join(",")),
        });
        outcome = outcome.with(Finding {
            target: target.to_string(),
            kind: FindingKind::Note,
            message: format!(
                "Open TCP port(s) detected at {}: {} — port-open only, NO service/version confirmation ({} port-open probes collapsed); run the individual exploit modules to verify",
                target,
                ports_csv.join(", "),
                module_count
            ),
            data: Some(serde_json::json!({
                ...
                "probes_by_port": open_port_modules,
                "status": "port-open-only",
            })),
        });
    }

    // One aggregated note per (port, banner-marker) fingerprint: the banner is
    // positive evidence the SERVICE is present; the list records which CVE
    // probes matched it.
    for ((port, marker), modules) in &banner_hits {
        let sample = match banner_samples.get(&(*port, *marker)) {
            Some(s) => s.clone(),
            None => String::new(),
        };
        crate::mprintln!(
            "{} '{}' banner at {}:{} — {} probe module(s) fingerprinted",
            "[?]".yellow(),
            marker,
            target,
            port,
        ...
        outcome = outcome.with(Finding {
            ...
            kind: FindingKind::Note,
            message: format!(
                "Service '{}' banner observed at {}:{} (fingerprint only; {} matching probe modules)",
                marker,
                target,
                port,
                modules.len()
...
enum ProbeResult {
    ...
    Unknown(String),
    /// A bare TCP connect succeeded — the port is open, nothing more is known.
    /// Collapsed per port by the caller instead of one note per CVE probe.
    OpenPort(u16),
    /// A service banner was read and matched the probe's marker (positive
    /// service fingerprint). Collapsed per (port, marker) by the caller.
    BannerMatch {
        port: u16,
        marker: &'static str,
        banner: String,
    ...
    match &probe.probe {
        ProbeType::Http {
            ...
            match client.get(&url).send().await {
                Ok(resp) => {
                    let server = crate::utils::network::header_string(resp.headers(), "server")
                        .to_lowercase();
                    ...
                    {
                        Ok(b) => b.to_lowercase(),
                        // A body-read failure must not be indistinguishable from
                        // "no markers present" — report it as a probe error.
                        Err(e) => {
                            return ProbeResult::Error(format!(
                                "body decode failed at {}: {e}",
                                base
                            ));
                        }
                    };
        ...
        ProbeType::Tcp { port } => {
            ...
            match crate::utils::network::tcp_connect(&addr, Duration::from_secs(3)).await {
                Ok(_) => ProbeResult::OpenPort(*port),
                Err(e) => ProbeResult::NotVulnerable(format!(
                    "{}:{} closed/unreachable: {e}",
                    target, port
                )),
            }
        }
        ProbeType::TcpBanner { port, marker } => {
            ...
            match crate::utils::network::tcp_connect(&addr, Duration::from_secs(3)).await {
                Ok(mut stream) => {
                    ...
                    {
                        Ok(Ok(n)) if n > 0 => {
                            let banner = match buf.get(..n) {
                                Some(slice) => String::from_utf8_lossy(slice).to_lowercase(),
                                None => String::new(),
                            };
                            if banner.contains(&marker.to_lowercase()) {
                                ...
                                ProbeResult::NotVulnerable(format!(
                                    "non-{} service at {}",
                                    marker, addr
                                ))
                            }
                        }
                        Ok(Ok(_)) => ProbeResult::NotVulnerable(format!("no banner at {}", addr)),
                        Ok(Err(e)) => {
                            ProbeResult::Error(format!("banner read failed at {}: {e}", addr))
                        }
                        Err(e) => {
                            ProbeResult::Error(format!("banner read timed out at {}: {e}", addr))
                        }
                    }
                }
...
fn build_probe_registry() -> Vec<Probe> {
    vec![
        ...
        p(
            ...
            https("/mifs/login.jsp", &["MobileIron", "Ivanti"]),
        ),
        p(
        ...
        p(
            ...
            http_port(8080, "/login", &["Jenkins"]),
        ),
        p(
            "exploits/frameworks/jenkins/jenkins_args4j_rce_cve_2024_23897",
            http_port(8080, "/login", &["Jenkins"]),
        ),
        p(
        ...
        p(
            "exploits/webapps/mcpjam/mcpjam_prompt_injection_cve_2026_10293",
            http("/", &["MCPJam"]),
        ),
        // ── Routers ──
        p(
        ...
        p(
            ...
            http_port(9443, "/", &["SafeLine", "safeline"]),
        ),
        p(
...
fn print_banner() {
    ...
    crate::mprintln_block!(
        ...
        format!(
            "{}",
            "║              Vulnerability Checker                         ║".cyan()
        ),
        format!(
            "{}",
            "║   Fingerprints target against all exploit module signatures ║".cyan()
        ),
        format!(
            "{}",
            "╚═══════════════════════════════════════════════════════════╝".cyan()
