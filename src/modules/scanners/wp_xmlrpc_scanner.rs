...
fn banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   WordPress XML-RPC Scanner                                  ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Detects xmlrpc.php + system.multicall + pingback primitives║".cyan()
    );
    crate::mprintln!(
        "{}",
        "╚══════════════════════════════════════════════════════════════╝".cyan()
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    if let Ok(r) = get {
        ...
        if body.contains("XML-RPC server accepts POST requests only") || s == 405 {
            crate::mprintln!(
                "{}",
                format!("[+] xmlrpc.php reachable (status {})", s)
                    .green()
                    .bold()
            );
            reachable = true;
            ...
            crate::mprintln!(
                "{}",
                format!("[~] {} -> 404, no WordPress XML-RPC here", endpoint).dimmed()
            );
            return Ok(outcome);
        } else {
            crate::mprintln!(
    ...
    if !reachable {
        crate::mprintln!(
            "{}",
            "[~] Endpoint did not confirm WordPress; continuing with POST tests anyway".dimmed()
        );
    }
    ...
    crate::mprintln!(
        "{}",
        format!("[*] POST system.listMethods -> {}", endpoint).cyan()
    );
    ctx.rate_limit(target).await;
    let resp = client
        .post(&endpoint)
    ...
    if let Ok(r) = resp {
        ...
        if body.contains("methodResponse") && body.contains("<string>") {
            ...
            crate::mprintln!(
                "{}",
                format!("[+] listMethods OK (status {}, ~{} methods)", s, count)
                    .green()
                    .bold()
            );
            if has_multicall {
                ...
                outcome.findings.push(Finding {
                    ...
                    message: format!(
                        "system.multicall enabled at {endpoint} — password-spray amplifier"
                    ),
                    data: None,
                });
            }
    ...
    if has_pingback {
        crate::mprintln!(
            "{}",
            "[*] (Skipping live pingback.ping probe — would require attacker callback URL.)"
                .dimmed()
        );
    }
    ...
    if has_multicall || has_pingback {
        crate::mprintln!(
            "{}",
            format!(
                "  Findings: multicall={} pingback={} — file as low/medium with amplification CVSS",
                has_multicall, has_pingback
            )
            .yellow()
        );
    } else if reachable {
