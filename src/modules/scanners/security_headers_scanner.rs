...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    // header_string renders a "<non-utf8>" sentinel for non-UTF-8 values
    // (instead of silently collapsing them) and "" when the header is absent.
    let get = |name: &str| -> Option<String> {
        let v = crate::utils::network::header_string(&headers, name);
        ...
        Some(v) => {
                ...
                .find_map(|p| p.trim().strip_prefix("max-age="))
            {
                Some(n) => match n.parse::<u64>() {
                    Ok(parsed) => parsed,
                    Err(e) => {
                        tracing::debug!("HSTS max-age '{n}' not numeric: {e}");
                        0
                    }
                },
                None => 0,
