                ...
                .await
                {
                    Ok(Some(sys_descr)) => {
                        ...
                        .await
                        {
                            ...
                            Ok(None) => {}
                            Err(e) => {
                                crate::meprintln!("[!] sysName query failed: {}", e);
                            }
                        }
                        match test_community(
                            ctx,
                        ...
                        .await
                        {
                            ...
                            Ok(None) => {}
                            Err(e) => {
                                crate::meprintln!("[!] sysLocation query failed: {}", e);
                            }
                        }

                        let (ev_host, ev_port) = match addr.rsplit_once(':') {
                            Some((h, p)) => match p.parse::<u16>() {
                                Ok(port_num) => (h.to_string(), port_num),
                                Err(e) => {
                                    tracing::warn!(
                                        "SNMP addr '{}' port '{}' is not numeric ({}); defaulting event port to 161",
                                        addr,
                                        p,
                                        e
                                    );
