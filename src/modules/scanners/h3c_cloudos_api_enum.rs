...
const DEFAULT_BASIC_AUTH: &str =
    "Basic eGJkNjFuYnJ4b3hmbGpqbHVhYjN0c3dteGd5ZWRteHl2emQwazIyeHlkY294Zmxq";
...
const CLOUDOS_ENDPOINTS: &[(&str, &str)] = &[
    ...
    (
        "/os/kinton/v1/keystone/types/UserProjectGroup/assigments",
        "User-project-group assignments",
    ),
    // Compute
    (
        "/os/compute/v1/v2/{tenant_id}/servers",
        "Compute servers list",
    ),
    (
        "/os/compute/v1/v2/{tenant_id}/flavors/detail",
        "Compute flavors",
    ),
    (
        "/os/compute/v1/cloudos/azones/{azone_id}",
        "Availability zones",
    ),
    // Image
    ("/os/image/v1/v2/images", "OS images list"),
    (
        "/os/kinton/v1/cloudos/imagetype/opsys/{os_type}",
        "Image types by OS",
    ),
    // Resource management
    (
        "/os/kinton/v1/cloudos/resource/norms/{serviceUuid}",
        "Resource norms",
    ),
    (
        "/os/kinton/v1/cloudos/resource/normConfi",
        "Resource norm config",
    ),
    (
        "/os/kinton/v1/quotaMange/saveQuotaManage",
        "Quota management",
    ),
];
...
fn display_banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   H3C CloudOS API Enumerator                                ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Probes identity, compute, image & resource APIs            ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Reports unauthenticated disclosure & default credentials   ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "╚══════════════════════════════════════════════════════════════╝".cyan()
...
pub fn info() -> ModuleInfo {
    ModuleInfo {
        ...
        description: "Probes H3C CloudOS management platform APIs (Kinton/Keystone identity, \
                      ...
                      requiring authentication. FOR AUTHORIZED TESTING ONLY."
            .to_string(),
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    .await? as u64;
    let tenant_id =
        cfg_prompt_default("tenant_id", "Tenant ID for compute endpoints", "default").await?;
    ...
    let service_uuid =
        cfg_prompt_default("service_uuid", "Service UUID (blank for none)", "").await?;
    let probe_discovery = cfg_prompt_yes_no(
        "probe_discovery",
        "Probe API discovery/management endpoints",
        true,
    )
    .await?;
    let try_default_creds = cfg_prompt_yes_no(
        "try_default_creds",
        "Try known default Basic auth header",
        true,
    )
    .await?;
    ...
    for s in schemes {
        ...
        match client.get(&probe_url).send().await {
            Ok(r) => {
                ...
                crate::mprintln!(
                    "{}",
                    format!("[+] Host reachable: {} (HTTP {})", probe_url, r.status()).green()
                );
                break;
            }
            Err(e) => {
                ...
                if status.is_success() {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            crate::meprintln!(
                                "{}",
                                format!("[-] Failed to read body from {}: {}", url, e).red()
                            );
                            String::new()
                        }
                    };
                    // A bare 2xx is not evidence: empty bodies, HTML login
                    // pages and SPA shells return 200 on benign hosts. Require
                    // a parseable JSON data payload (the shape every CloudOS
                    // API returns) before flagging.
                    if !is_api_disclosure(&body) {
                        if verbose {
                            crate::mprintln!(
                                "{}",
                                format!(
                                    "[-] {} ({}) -> {} (2xx but body is not a data payload — not flagged)",
                                    path, label, status
                                )
                                .dimmed()
                            );
                    ...
                    crate::mprintln!(
                        "{}",
                        format!("[-] {} ({}) -> {}", path, label, status).dimmed()
                    );
                }
            }
            Err(e) => {
            ...
            match client.get(&url).send().await {
                Ok(resp) => {
                    ...
                    if status.is_success() {
                        ...
                        // Empty bodies and generic HTML login/landing shells
                        // return 200 on most web hosts — require content that
                        // is neither empty nor a login shell before flagging.
                        if body.trim().is_empty() || looks_like_login_shell(&body) {
                            if verbose {
                                crate::mprintln!(
                                    "{}",
                                    format!(
                                        "[-] {} ({}) -> {} (2xx but body is empty or a login/landing shell — not flagged)",
                                        path, label, status
                                    )
                                    .dimmed()
                                );
                        ...
                        if verbose {
                            crate::mprintln!(
                                "{}",
                                format!("[-] {} ({}) -> {} (properly gated)", path, label, status)
                                    .dimmed()
                            );
                        }
                    } else if verbose {
    ...
    if try_default_creds {
        ...
        for (path_template, label) in CLOUDOS_ENDPOINTS {
            ...
            if unauth_success.contains(&url) {
                if verbose {
                    crate::mprintln!(
                        "{}",
                        format!(
                            "[-] {} ({}) already unauthenticated — skipping default-cred check",
                            path, label
                        )
                        .dimmed()
                    );
            ...
            ctx.rate_limit(&host).await;
            let req = client.get(&url).header("Authorization", DEFAULT_BASIC_AUTH);
            match req.send().await {
                Ok(resp) => {
                    ...
                    if status.is_success() {
                        ...
                        // The same positive-evidence rule as the unauthenticated
                        // phase: a 2xx without a JSON data payload (empty body,
                        // HTML login shell, SPA index) does not prove the
                        // default credential granted access to anything.
                        if !is_api_disclosure(&body) {
                            if verbose {
                                crate::mprintln!(
                                    "{}",
                                    format!(
                                        "[-] {} ({}) -> {} with default creds (body is not a data payload — not flagged)",
                                        path, label, status
                                    )
                                    .dimmed()
                                );
                        ...
                        crate::mprintln!(
                            "{}",
                            format!("[-] {} ({}) -> {} with default creds", path, label, status)
                                .dimmed()
                        );
                    }
                }
            ...
            match req.send().await {
                Ok(resp) => {
                    ...
                    if status.is_success() {
                        ...
                        // Same rule as the unauthenticated discovery phase.
                        if body.trim().is_empty() || looks_like_login_shell(&body) {
                            if verbose {
                                crate::mprintln!(
                                    "{}",
                                    format!(
                                        "[-] {} ({}) -> {} with default creds (body is empty or a login/landing shell — not flagged)",
                                        path, label, status
                                    )
                                    .dimmed()
                                );
        ...
        if default_cred_hits == 0 {
            crate::mprintln!(
                "{}",
                "[*] Default credentials did not grant additional access.".cyan()
            );
        } else {
    ...
    if hits == 0 {
        crate::mprintln!(
            "{}",
            "[*] No unauthenticated or default-credential disclosures found.".cyan()
        );
    } else {
        crate::mprintln!(
            "{}",
...
/// True when a 2xx response body is positive evidence of a data disclosure:
/// a parseable JSON payload (the shape every CloudOS data API returns).
/// Empty bodies and HTML login/SPA shells return 200 on benign hosts and must
/// never be flagged on status alone.
fn is_api_disclosure(body: &str) -> bool {
    serde_json::from_str::<serde_json::Value>(body.trim()).is_ok()
}

/// True when a non-JSON 2xx body still looks like a generic HTML login or
/// landing shell (which returns 200 to everyone) rather than real content.
fn looks_like_login_shell(body: &str) -> bool {
    let lower = body.to_lowercase();
    let html = lower.contains("<html") || lower.contains("<!doctype html");
    html && (lower.contains("login") || lower.contains("sign in") || lower.contains("password"))
}
    ...
    match serde_json::from_str::<serde_json::Value>(body) {
        Ok(val) => match &val {
            ...
            serde_json::Value::Array(arr) => {
                crate::mprintln!(
                    "    {}",
                    format!("array with {} element(s)", arr.len()).dimmed()
                );
            }
            _ => {}
        },
        Err(e) => {
            // Not an operational failure (the caller already gated on JSON
            // data payloads where required) — log it so non-JSON bodies are
            // never silently skipped.
            tracing::debug!("display_json_summary: body is not JSON: {e}");
        }
    }
}
