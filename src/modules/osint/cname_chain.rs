...
fn banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   CNAME Chain Follower (OSINT)                               ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Recursive CNAME → A; tags terminal cloud providers         ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "╚══════════════════════════════════════════════════════════════╝".cyan()
...
pub fn info() -> ModuleInfo {
    ModuleInfo {
        ...
        references: vec!["https://datatracker.ietf.org/doc/html/rfc1034#section-3.6.2".to_string()],
...
async fn dns_lookup(
    ...
    // Drive the background task via the framework's tracked spawn so it is
    // registered with the run's JoinSet and aborted on completion/cancellation
    // instead of leaking as a detached task per lookup.
    ctx.spawn(async move {
        if let Err(e) = bg.await {
            tracing::error!("DNS background driver task failed: {}", e);
        }
    });
    let qname = Name::from_str_relaxed(name)?;
    let resp = timeout(
        Duration::from_secs(DNS_TIMEOUT_SECS),
        client.query(qname, DNSClass::IN, rtype),
    )
    .await??;
...
/// Returns (chain, terminal-A resolution state, mid-chain lookup error).
/// `chain_error` is Some(msg) when a CNAME hop lookup failed at the transport
/// level, which means the chain is INCOMPLETE — the caller must not treat the
/// truncated chain as authoritative.
async fn chain_for(
    ctx: &ModuleCtx,
    host: &str,
    resolver: &str,
    ...
    for _ in 0..10 {
        match dns_lookup(ctx, &current, RecordType::CNAME, resolver).await {
            Ok(ans) => {
                let next = ans
                    .into_iter()
            ...
            Err(e) => {
                tracing::debug!("CNAME lookup for {} failed: {}", current, e);
                chain_error = Some(format!("CNAME lookup for {current} failed: {e}"));
                break;
            }
        }
    }
    ...
    for attempt in 0..3 {
        match dns_lookup(ctx, host, RecordType::A, resolver).await {
            Ok(v) => {
                resolves = Some(!v.is_empty());
                break;
            }
            Err(e) => {
                tracing::debug!(
                    "A lookup for {} failed (attempt {}): {}",
                    host,
                    attempt + 1,
                    e
                );
    ...
    (chain, resolves, chain_error)
}

fn tag_provider(host: &str) -> Option<&'static str> {
    let lh = host.to_ascii_lowercase();
    for (suf, lbl) in SUFFIXES {
        if lh.ends_with(&suf.to_ascii_lowercase()) {
            return Some(lbl);
        }
    }
    None
...
/// Resolve one host's CNAME chain — the per-item future for the parallel
/// stream in `run`. Deliberately a named `async fn` rather than a closure
/// wrapping an `async move` block: an opaque async block that borrows the
/// enclosing scope (here `ctx` and the resolver) cannot be generalized over
/// lifetimes when it sits inside the async-trait-boxed module `run`, and rustc
/// rejects it with "implementation of FnOnce is not general enough". A
/// function-item future carries explicit lifetime parameters and checks fine.
/// Concurrency and result ordering are unchanged.
async fn resolve_host_chain(
    ctx: &ModuleCtx,
    host: String,
    resolver: &str,
) -> (String, Vec<String>, Option<bool>, Option<String>) {
    let (chain, resolves, chain_error) = chain_for(ctx, &host, resolver).await;
    (host, chain, resolves, chain_error)
}
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let from_file =
        cfg_prompt_yes_no("from_file", "Treat target as a path to a host list?", false).await?;
    ...
    // The per-host work is a named `async fn` (see `resolve_host_chain`) — a
    // closure returning an opaque async block that borrows this scope cannot
    // be lifetime-generalized inside the async-trait-boxed `run`.
    let resolved: Vec<(String, Vec<String>, Option<bool>, Option<String>)> = futures::stream::iter(
        hosts
            .iter()
            .cloned()
            .map(|host| resolve_host_chain(ctx, host, &resolver)),
    )
    .buffered(CNAME_CONCURRENCY)
    .collect()
    .await;

    for (host, chain, resolves, chain_error) in resolved {
        ...
        // A mid-chain CNAME lookup failure means the chain above is INCOMPLETE —
        // surface it at operator-visible level (outside batch mode) and carry it
        // into the finding message so a truncated chain is never authoritative.
        let chain_error_suffix = match &chain_error {
            Some(err) => {
                if !is_batch_mode() {
                    crate::mprintln!("{}", format!("[?] {host}: {err}").yellow());
                }
                format!("; CNAME follow interrupted ({err})")
            }
            None => String::new(),
        };
        ...
        let label = match (provider, dangling_tag) {
            ...
            (Some(ref p), false) if lookup_failed => format!("[?] {} (resolution unknown)", p)
                .yellow()
                .to_string(),
        ...
        let resolves_str = match resolves {
            Some(true) => "true",
            Some(false) => "false",
            None => "unknown",
        };
        ...
        if dangling_tag {
            outcome.findings.push(Finding {
                ...
                message: format!(
                    "Dangling CNAME {host} -> {chain_str} (terminal does not resolve){chain_error_suffix}"
                ),
                data: None,
            });
            ...
            outcome.findings.push(Finding {
                ...
                message: format!("CNAME {host} -> {chain_str} (terminal resolution unconfirmed; A lookup failed, requires re-check){chain_error_suffix}"),
                data: None,
            });
            ...
            outcome.findings.push(Finding {
                target: host.clone(),
                kind: FindingKind::Note,
                message: format!("CNAME {host} -> {chain_str} (provider: {p}){chain_error_suffix}"),
                data: None,
            });
        } else if let Some(err) = chain_error {
            // Chain was truncated by a transport error and nothing else was
            // reported for this host — record an explicit Note.
            outcome.findings.push(Finding {
                target: host.clone(),
                kind: FindingKind::Note,
                message: format!("CNAME chain for {host} incomplete: {err}"),
                data: None,
            });
        }
    }
