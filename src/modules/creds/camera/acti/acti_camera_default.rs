...
pub async fn check_ftp(config: &Config) -> Result<Option<(ServiceType, String, String)>> {
    ...
    for (username, password) in &config.credentials {
        ...
        match AsyncFtpStream::connect_with_stream(tcp_stream).await {
            Ok(mut ftp) => {
                // Distinguish an explicit rejection from a transport error in the
                // trace log, but treat both as "this credential did not work" and
                // keep iterating (same semantics as before, minus the `.is_ok()`
                // conflation).
                match ftp.login(username, password).await {
                    Ok(()) => {
                        crate::mprintln!(
                            "{}",
                    ...
                    Err(e) => {
                        tracing::trace!(target = %config.target, port = config.port, user = %username, "FTP login rejected/failed: {}", e);
                        if let Err(e) = ftp.quit().await {
                            crate::meprintln!("[!] FTP quit failed: {}", e);
                        }
                    }
...
pub fn check_ssh_blocking(config: &Config) -> Result<Option<(ServiceType, String, String)>> {
    ...
    for (username, password) in &config.credentials {
        ...
        let stream = match crate::utils::network::blocking_tcp_connect(
            &socket_addr,
            Duration::from_secs(DEFAULT_TIMEOUT_SECS),
        ) {
            Ok(s) => s,
            Err(e) => {
                tracing::debug!("ACTi SSH TCP connect to {socket_addr} failed: {e}");
                continue;
            }
        };
        ...
        match session.userauth_password(username, password) {
            ...
            Ok(()) => {
                tracing::trace!(user = %username, "SSH auth completed but session not authenticated");
            }
            Err(e) => {
                tracing::trace!(target = %config.target, user = %username, "SSH auth rejected: {}", e);
            }
        }
    }
...
pub fn check_telnet_blocking(config: &Config) -> Result<Option<(ServiceType, String, String)>> {
    ...
    for (username, password) in &config.credentials {
        ...
        let address = normalize_target(&config.target, config.port);
        // rsplitn(2, ':') yields at most two parts; peel them via the iterator
        // (no indexing) and surface a malformed port instead of a silent default.
        let mut rsplit = address.rsplitn(2, ':');
        let port_str = match rsplit.next() {
            Some(p) => p,
            None => continue,
        };
        let host = match rsplit.next() {
            Some(h) => h,
            None => continue,
        };
        let port: u16 = match port_str.parse() {
            ...
            Err(e) => {
                tracing::debug!(
                    "Telnet port '{}' invalid ({}), defaulting to 23",
                    port_str,
                    e
                );
                23
        ...
        let tcp_stream = match crate::utils::network::blocking_tcp_connect(
            &socket_addr,
            Duration::from_secs(DEFAULT_TIMEOUT_SECS),
        ) {
            Ok(s) => s,
            Err(e) => {
                tracing::debug!("ACTi Telnet TCP connect to {socket_addr} failed: {e}");
                continue;
            }
        };
        ...
        match telnet.read_timeout(Duration::from_millis(800)) {
            ...
            // A timeout event is a normal non-data outcome, not an error.
            Ok(other) => {
                tracing::trace!(addr = %socket_addr, "Telnet read produced no data event: {:?}", other);
            }
            Err(e) => {
                tracing::trace!(addr = %socket_addr, "Telnet read failed: {}", e);
            }
        }
    }
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    // A service check returning Err is a real transport/internal failure —
    // surface it rather than silently treating it as "no credentials found".
    for (svc, res) in [
        ("FTP", ftp_res),
        ("SSH", ssh_res),
        ("Telnet", telnet_res),
        ...
        match res {
            Ok(Some((service, user, pass))) => found_credentials.push((service, user, pass)),
            Ok(None) => {}
            Err(e) => crate::meprintln!("[!] {} service check failed: {}", svc, e),
        }
    }
