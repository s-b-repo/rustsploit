...
fn display_banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   System Ulimit Configuration Utility                     ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "║   Raises file descriptor limits for brute forcing         ║".cyan()
    );
    crate::mprintln!(
        "{}",
        "╚═══════════════════════════════════════════════════════════╝".cyan()
...
async fn raise_ulimit() -> Result<()> {
    ...
    let (current_soft, current_hard) = match get_current_limits() {
        ...
        Err(e) => {
            crate::mprintln!(
                "{}",
                format!("[-] Failed to get current limits: {}", e).red()
            );
            (0, 0)
        }
    };
    ...
    if current_soft >= TARGET_FILE_LIMIT {
        crate::mprintln!(
            "{}",
            format!("[+] Open file limit already at {} or higher.", current_soft)
                .green()
                .bold()
        );
        return Ok(());
    ...
    crate::mprintln!(
        "{}",
        format!(
            "[*] Attempting to raise open file limit to {}",
            TARGET_FILE_LIMIT
        )
        .cyan()
    );
    ...
    match set_file_limit(target_soft, target_hard) {
        Ok(()) => {
            crate::mprintln!(
                "{}",
                format!("[+] Successfully set file limit to {}", target_soft)
                    .green()
                    .bold()
            );
        }
        Err(e) => {
            ...
            crate::mprintln!(
                "{}",
                format!("[-] Could not set to {}: {}", TARGET_FILE_LIMIT, e).yellow()
            );
            ...
            if current_hard > current_soft {
                crate::mprintln!(
                    "{}",
                    format!(
                        "[*] Trying to raise soft limit to hard limit ({})...",
                        current_hard
                    )
                    .cyan()
                );
                match set_file_limit(current_hard, current_hard) {
                    Ok(()) => {
                        crate::mprintln!(
                            "{}",
                            format!("[+] Raised soft limit to {}", current_hard).green()
                        );
                    }
                    Err(e2) => {
                        crate::mprintln!(
                            "{}",
                            format!("[-] Could not raise soft limit: {}", e2).red()
                        );
                        crate::mprintln!(
                            "{}",
                            "[!] Try running as root or adjust /etc/security/limits.conf".yellow()
                        );
                    }
                }
            } else {
                ...
                crate::mprintln!(
                    "{}",
                    "[!] To increase further, run as root or edit /etc/security/limits.conf"
                        .yellow()
                );
            }
        }
    ...
    match get_current_limits() {
        Ok((new_soft, new_hard)) => {
            crate::mprintln!(
                "{}",
                format!("[*] New limits - Soft: {}, Hard: {}", new_soft, new_hard).cyan()
            );
            if new_soft >= TARGET_FILE_LIMIT {
                crate::mprintln!(
                    "{}",
                    "[+] File descriptor limit successfully raised!"
                        .green()
                        .bold()
                );
                ...
                crate::mprintln!(
                    "{}",
                    format!("[+] Limit raised from {} to {}", current_soft, new_soft).green()
                );
            } else {
        ...
        Err(e) => {
            crate::mprintln!(
                "{}",
                format!("[-] Could not verify new limits: {}", e).yellow()
            );
        }
    }
    ...
    crate::mprintln!(
        "{}",
        "To raise limits in your shell before running rustsploit:".dimmed()
    );
    crate::mprintln!("{}", "  ulimit -n 65535".white());
    crate::mprintln!(
        "{}",
        "Or to make permanent, add to /etc/security/limits.conf:".dimmed()
    );
