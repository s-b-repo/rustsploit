...
async fn probe(host: &str, port: u16, community: &str, timeout: Duration) -> LoginResult {
    ...
    let n = match tokio::time::timeout(timeout, sock.recv_from(&mut buf)).await {
        ...
        Err(e) => {
            // For UDP a missing response usually means the datagram was dropped,
            // the host is filtered, or the agent is slow — NOT that the community
            // string was rejected (SNMPv2c agents stay silent for bad communities,
            // there is no negative ack). Classify as a retryable error so the
            // engine retries the probe before concluding anything.
            tracing::debug!("SNMP recv timed out: {e}");
            return LoginResult::Error {
                message: format!("snmp recv timeout (response inconclusive): {e}"),
                retryable: true,
            };
        }
    };
