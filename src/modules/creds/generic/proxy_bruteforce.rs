...
async fn try_http_connect_auth(
    ...
    match http_status_code(&resp) {
        ...
        None => LoginResult::Error {
            message: format!("non-HTTP response: {}", resp.lines().next().unwrap_or("")),
            retryable: false,
        },
    }
}
...
async fn try_http_forward_auth(
    ...
    if matches!(code, Some(c) if (200..=299).contains(&c)) && resp.contains("origin") {
        ...
        LoginResult::Error {
            message: format!("HTTP {}", resp.lines().next().unwrap_or("")),
            retryable: false,
        }
    }
}
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let timeout_ms: u64 = cfg_prompt_default("timeout", "Timeout (ms)", "5000")
        .await?
        .parse()
        .unwrap_or(5000);
