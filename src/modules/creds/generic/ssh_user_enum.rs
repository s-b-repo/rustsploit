...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
        ...
        && cfg_prompt_yes_no("save_results", "Save valid users to file?", true).await?
    {
        ...
        // Reduce the operator/config-supplied path to its basename and reject
        // empty or dot-prefixed names. A value like `../../etc/cron.d/x` (which
        // can reach run() via custom_prompts/options in API/multi-tenant mode)
        // would otherwise truncate arbitrary writable paths (same defence as
        // ssh_spray's output file).
        let output_path = match std::path::Path::new(&raw_output_path).file_name() {
            Some(name) => name.to_string_lossy().into_owned(),
            None => {
                return Err(anyhow!(
                    "invalid output file '{}': no filename component",
                    raw_output_path
                ));
            }
        };
        if output_path.is_empty() || output_path.starts_with('.') {
            return Err(anyhow!(
                "invalid output file name '{}' (must be a plain, non-dotfile name in the current directory)",
                output_path
            ));
        }
        let mut file = {
