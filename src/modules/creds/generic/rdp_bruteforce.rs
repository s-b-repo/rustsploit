...
pub async fn run(target: &str) -> Result<()> {
    ...
    let save_results = if api_mode { true } else { prompt_yes_no("Save results to file?", true).unwrap_or(true) };
    let save_path = if save_results {
        Some(config_api.output_file.clone().unwrap_or_else(|| {
            if api_mode { "rdp_results.txt".to_string() } else { prompt_default("Output file name", "rdp_results.txt").unwrap_or_else(|_| "rdp_results.txt".to_string()) }
        }))
    } else {
        None
    };
    ...
    let security_level = if api_mode { RdpSecurityLevel::Auto } else { RdpSecurityLevel::prompt_selection()? };
