...
pub async fn resolve_antispoofing_key(model_id: Option<u32>) -> Result<PublicKey> {
    ...
    // 2. Local key cache (persisted across sessions).
    if let Some(mid) = model_id {
        if let Some(cached) = cached_key(mid) {
            return crypto::parse_provider_pubkey_b64(&cached);
        }
    }

    // 3. Google Nearby Devices metadata API (if configured).
    if let Some(mid) = model_id
    ...
    Err(anyhow!(
        ...
        match model_id {
            Some(m) => format!(" for model 0x{m:06X}"),
            None => String::new(),
        }
    ))
}
...
pub async fn fetch_from_metadata_api(model_id: u32) -> Result<Option<String>> {
    ...
    let api_key = match go.get("gfp_api_key").await {
...
fn extract_antispoofing_key_from_response(body: &str) -> Option<String> {
    match serde_json::from_str::<serde_json::Value>(body) {
        ...
        Err(e) => {
            tracing::debug!(
                "wpair: metadata API response is not valid JSON ({e}); treating as raw base64"
            );
...
static KEY_CACHE: std::sync::OnceLock<std::sync::Mutex<HashMap<u32, String>>> =
    std::sync::OnceLock::new();

fn key_cache() -> &'static std::sync::Mutex<HashMap<u32, String>> {
    KEY_CACHE.get_or_init(|| {
        let mut map = HashMap::new();
        if let Some(path) = key_cache_path() {
            if path.exists() {
                match std::fs::read_to_string(&path) {
                    Ok(data) => {
                        for line in data.lines() {
                            ...
                            if let Some((id_str, key)) = trimmed.split_once(' ') {
                                match u32::from_str_radix(id_str, 16) {
                                    Ok(id) => {
                                        map.insert(id, key.to_string());
                                    }
                                    Err(e) => tracing::warn!(
                                        "wpair: key-cache row '{trimmed}' has a malformed \
                                         model id ({e}) — skipped"
                                    ),
                                }
                            }
                        }
                        tracing::debug!("wpair: loaded {} cached anti-spoofing keys", map.len());
                    }
                    Err(e) => {
                        tracing::debug!("wpair: could not read key cache {}: {e:#}", path.display())
                    }
                }
            }
        }
...
fn key_cache_path() -> Option<std::path::PathBuf> {
    let home = std::env::var("HOME").ok()?;
    let mut path: std::path::PathBuf = home.into();
    path.push(".rustsploit");
    path.push("wpair_keys");
    Some(path)
}

fn cache_key(model_id: u32, key: &str) {
    let mut guard = match key_cache().lock() {
        Ok(g) => g,
        Err(poisoned) => {
            tracing::warn!("wpair: key cache mutex poisoned — recovering its contents");
            poisoned.into_inner()
        }
    };
    guard.insert(model_id, key.to_string());
    if let Some(path) = key_cache_path() {
        let mut content = String::from("# WhisperPair anti-spoofing key cache\n");
        for (id, k) in guard.iter() {
            content.push_str(&format!("{:06X} {}\n", id, k));
        }
        if let Err(e) = std::fs::write(&path, content) {
            tracing::debug!("wpair: failed to write key cache: {}", e);
        }
    }
}

fn cached_key(model_id: u32) -> Option<String> {
    key_cache().lock().ok()?.get(&model_id).cloned()
}
