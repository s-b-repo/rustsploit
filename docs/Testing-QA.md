...
A clean `cargo check` is required: **0 errors and 0 warnings** (the zero-warning gate is enforced across the whole tree — fix the cause, never suppress). Also run the banned-pattern audit:

```bash
scripts/audit-bad-patterns.sh --strict     # must exit 0
scripts/check-docs.sh                      # docs links + staleness gate
```
...
the `inventory` crate — there is no `build.rs` codegen.
All 468 modules are auto-discovered at link time. If a new module fails to
...
Run all unit tests (binary crate — tests run via the bin target):
```bash
cargo test --bin rustsploit
# or a single module/test by name filter:
cargo test --bin rustsploit cyclic
```
