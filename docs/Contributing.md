...
| OSINT | `src/modules/osint/` |
| Post-exploitation | `src/modules/post/` |
...
- See [`BAD_PATTERNS.md`](BAD_PATTERNS.md) for the complete 136-pattern audit checklist, enforced by `scripts/audit-bad-patterns.sh --strict` (the only sanctioned exemption is an inline `// audit-allow: <reason>` comment).
