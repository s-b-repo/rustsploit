...
| `api.rs` | `MAX_REQUEST_BODY` | 2 MiB | API request body limit |
...
For custom character validation, compile the regex once (module scope, never in a hot path) and propagate any compile error:
```rust
use std::sync::LazyLock;
use regex::Regex;

static TARGET_CHARS: LazyLock<Result<Regex, regex::Error>> =
    LazyLock::new(|| Regex::new(r"^[a-zA-Z0-9.\-_:\[\]]+$"));

if let Ok(re) = TARGET_CHARS.as_ref() {
    if !re.is_match(target) {
        return Err(anyhow!("Invalid characters in target"));
    }
} else {
    return Err(anyhow!("Target charset regex failed to compile"));
}
```
...
loop {
    ...
    if attempts > MAX_ATTEMPTS {
        crate::mprintln!("Too many invalid attempts. Using default.");
        return Ok(default);
    }
    // prompt logic
}
...
- **`RequestBodyLimitLayer`** — prevents DoS via oversized payloads (2 MiB max, `MAX_REQUEST_BODY`).
...
When `setg honeypot_detection` is on (default), the scheduler runs a fast
honeypot probe ahead of the module body and skips targets that look like
honeypots:

- `crate::utils::network::quick_honeypot_check` — **30 common ports**, 200 ms total budget
- If **11 or more** ports respond, the target is flagged as a likely honeypot
- Suppressed in mass-scan mode (the per-host probe would itself be the slow path)
- Callable from module code:

```rust
use crate::utils::network::quick_honeypot_check;
quick_honeypot_check(&ip).await;
```
...
IP exclusions are handled by `crate::exclusions::shared()`, a
...
This applies to module code as well — historical `.expect("slice of length N was checked")` patterns are converted to `try_into().with_context(|| ...)?` even when the invariant truly held, so a future reader doesn't need to verify by hand. **Note that the `_or(default)` / `_or_default()` / `_or_else(|| ...)` family is NOT fine** — it is banned by the audit matrix ([BAD_PATTERNS.md](BAD_PATTERNS.md) A3–A5): a silent fallback lies about what happened; surface the error or propagate it.

---

## Supply-chain audit (point-in-time snapshot, v0.4.9 era)

Historical result — re-run `cargo audit` against the current `Cargo.lock` before relying on it.
