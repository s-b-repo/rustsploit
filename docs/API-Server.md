...
At every startup the server also prints a **one-time enrollment token** (24 random bytes, URL-safe base64). It is the only way to bootstrap a new client identity — POST the client's PQ public keys plus this token to `POST /pq/register-key`; the token is consumed on first use and held only in memory:
...
**Not implemented / compat shims**

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/api/run/all` (alias `POST /api/run_all`) | **Not implemented** — returns `501 NOT_IMPLEMENTED` |
| `POST` | `/api/shell` | **Not implemented** — returns `501 NOT_IMPLEMENTED`. Use the individual REST routes or the WebSocket RPC surface instead. |
| `POST` | `/api/modules/{m}/check` | ArcticFox-compat shim — the `Module` trait has no separate `check()`; this simply runs the module. Retained for old clients. |
...
> **Note:** Rustsploit is exploitation-only — there is no non-destructive `check` endpoint or phase. `POST /api/shell` and `POST /api/run/all` are **not implemented** (501).
...
WebSocket messages use the same JSON request/response format as REST endpoints. The WebSocket transport is ideal for long-running operations, real-time job monitoring, and persistent client connections. The WS RPC surface mirrors the REST routes plus extras (`check_module` compat, `spool_*`, `list_results`, `export`, …) — see `src/ws.rs` for the authoritative method list.
...
| Export engagement data | `GET /api/export?format=json\|csv\|summary` |
