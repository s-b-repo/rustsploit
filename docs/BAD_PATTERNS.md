...
> **`scripts/audit-bad-patterns.sh` is the executable form of this catalogue** —
> 136 regex checks applied across `src/`. It prints per-pattern hit counts and
> offending lines; `--strict` exits non-zero on any hit in the strict sections.
>
> ```sh
> scripts/audit-bad-patterns.sh                                  # whole tree, report only
> scripts/audit-bad-patterns.sh --strict                         # gate: exit 1 on any strict hit
> scripts/audit-bad-patterns.sh --strict --files <changed.rs …>  # gate a change / PR
> scripts/audit-bad-patterns.sh --section H                      # run a single section
> ```
>
> Dated whole-tree snapshots live in `reports/audit-report.md`.

A complete checklist of patterns that must not appear in module code. Every
entry has:

- **Pattern (regex)** — what the audit script (and `grep -E`) finds
- **Why it's bad** — what concretely goes wrong if it slips in
- **Fix** — the explicit alternative

CI / pre-merge review treats any non-zero strict count as a hard failure.

> **Exemptions.** The only sanctioned exemption marker is an inline
> `// audit-allow: <reason>` comment on the offending line — visible,
> reviewable, and grep-able. `#[allow(...)]` attributes remain banned (C1):
> they suppress the compiler, not just the audit.

> **Scope.** Module code under `src/modules/exploits/`, `src/modules/scanners/`,
> `src/modules/creds/`, `src/modules/osint/`, `src/modules/post/`,
> `src/modules/plugins/`. The
...
## The audit script is the source of truth

The tables above are the human-readable rationale for each section. The
executable matrix lives in `scripts/audit-bad-patterns.sh` (15 sections, A–P,
136 patterns) and is the authority for what CI enforces — when a rule changes,
change the script first, then this document.

**Script contract:**

- Walks `src/**/*.rs` by default; `--files <list>` restricts the scan (PR gate).
- `--strict` exits `1` on any hit in the strict sections (A panics, B
  swallowing, C lint suppression, L crypto, M injection, N UB/concurrency,
  O performance, P API hygiene); non-strict sections report without failing.
- `--section <X>` runs a single section.
- Honours inline `// audit-allow: <reason>` markers as the only exemption
  mechanism.

**Typical gate:**

```sh
# Pre-commit — whole tree
scripts/audit-bad-patterns.sh --strict

# Pre-merge — only the files in your change
scripts/audit-bad-patterns.sh --strict --files \
  src/modules/scanners/my_new_scanner.rs
```
...
The strict gate must return **0** for every entry in the matrix above on the
current tree; any hit fails the build.
