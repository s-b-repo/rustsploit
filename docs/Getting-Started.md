...
This starts the PQ-encrypted API server on port 8080. On first run it generates a host key pair at `~/.rustsploit/pq_host_key` and prints its fingerprint. At every startup the server also prints a **one-time enrollment token** — the only way to add a new client to `~/.rustsploit/pq_authorized_keys` (POST the client's PQ public keys + token to `POST /pq/register-key`; the token is consumed on first use). No TLS or API keys — authentication uses SSH-style post-quantum identity keys. See [API Server](API-Server.md) and [API Usage Examples](API-Usage-Examples.md) for details.
