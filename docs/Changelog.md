...
The Bluetooth surface goes from 25 modules (1 exploit + 3 scanners + 2 creds + 19 partial) to **35 cataloged modules** (30 exploits + 3 scanners + 2 creds), backed by 8 new protocol-stack codecs in `src/bluetooth/` (RFCOMM, OBEX, BNEP, AVDTP, HFP, SMP+LE-SC, Find My, Airoha vendor HCI). 10 new exploit modules added (BlueBug, BlueSnarf, CarWhisperer, AVDTP hijack, Find My clone, Airoha RCE, SMP off-curve, BlueBorne full vector set, BleedingTooth full CVE set, HID-over-GATT with real Report Map). 9 bug fixes in existing modules (`le_legacy_crack` real SMP addresses, `ctkd_probe` active overwrite, Mesh Beacon AD 0x2B parser, Sweyntooth/Braktooth CVE mapping, BlueFrag stage-2 payload, `mode_confusion` AES-CCM downgrade, `invalid_curve` description trim, matrix expansion, `wpair` planted-key → `inventory` sidecar → `classic_pin` follow-up). Plus a framework-wide module audit that fixed 128 files across all categories, the jenkins CVE-ID rename, and the `get_or<T>()` Display bound.

**Stats:** 468 → 477 modules. Build clean (0 errors, 0 warnings). 0 new strict BAD_PATTERNS violations. `check-docs: clean`.

Full details: [`changelogs/releases/CHANGELOG-0.5.1.md` § v0.5.1 (2026-09-07)](../changelogs/releases/CHANGELOG-0.5.1.md).

> **Version-lineage note:** the v0.5.x labels were reused across several
> 2026-05 → 2026-07 releases (duplicate `v0.5.0` / `v0.5.1` headings appear
> below). Entries are strictly newest-first — match releases by **date**, not
> by version label alone. The full per-release text for the newest entry also
> lives in [`changelogs/changelog-latest.md`](../changelogs/changelog-latest.md).

---

## v0.5.0 (2026-06-13) — Upstream ports + mass-scan fixes + HTTP pooling + per-run auto-save
