...
Use the shared `cfg_prompt_*` helpers from `crate::utils`, which respect the priority chain (per-request custom_prompts → RunContext target → global options → interactive stdin; in `api_mode`/batch mode they return defaults instead of blocking):
```rust
...
All bruteforce modules use the shared engine (`crate::utils::bruteforce`):

| Function | Use Case |
|----------|----------|
| `run_bruteforce()` / `run_bruteforce_with_abort()` / `run_bruteforce_streaming()` | Single-target credential testing with concurrency, progress, retry |
| `run_subnet_bruteforce()` | CIDR subnet scanning with per-host credential testing |
| `generate_combos_mode()` | Generate user/password pairs (combo or linear mode) |

Return `LoginResult` (`Success` / `AuthFailed` / `Error { message, retryable }`) from probes — the engine handles semaphores, progress reporting, lockout detection, and credential routing. Avoid custom concurrency.

---

## IPv6 Support

Normalize operator input through `crate::utils::target` (`normalize_target`), which handles IPv4/IPv6/hostname forms. When formatting `host:port` pairs for the wire, wrap IPv6 literals in brackets (`[::1]:22`).
...
## Available Credential Modules (34 total)
...
| `proxy_bruteforce` | 3128, 8080 | Proxy CONNECT / Basic auth | Upstream proxy credential testing |
| `fortinet_bruteforce` | 443 | FortiOS web login | CSRF token extraction, realm support |
| `h3c_oem_kvm_bruteforce` | 443 | H3C OEM KVM login | Default credential check for OEM KVM consoles |
| `h3c_redfish_session_spray` | 443 | Redfish session auth | Password spray over Redfish sessions |
| `m365_activesync_spray` | 443 | Microsoft ActiveSync Basic auth | Password spray with lockout awareness |
...
| Module | Transport | Auth Method | Features |
|--------|-----------|-------------|----------|
| `classic_pin` | BR/EDR | Legacy PIN pairing | PIN recovery against Classic Bluetooth devices |
| `le_legacy_crack` | BLE | LE Legacy Pairing | TK/STK recovery against legacy pairing |
...
All 34 credential modules support mass scanning via the framework's multi-target dispatcher. The framework automatically handles:
