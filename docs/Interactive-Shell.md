...
| `tommy` | `guide` | Interactive guided walkthrough / tutorial |
| `db_import <file>` | `nmap_import`, `di` | Import nmap scan results into the workspace |
| `save_profile <file>` | `sp` | Save session state (module, target, options) to a profile |
| `load_profile <file>` | `lp` | Load a saved session profile |
| `sessions` | `ss` | List active sessions |
| `socks` | | SOCKS proxy management |
...
> **Note:** `random`, `0.0.0.0/0`, and bare `0.0.0.0` all trigger the
> full-internet sweep path. Each sweep prints an advisory (effective port,
> concurrency, host cap, timeout, exclusion count) and asks for confirmation on
> an interactive terminal; API / piped runs proceed without prompting.
...
- **Random** (default) — `t random` / `t 0.0.0.0/0` / `t 0.0.0.0`: walks the
  public IPv4 space in a ZMap-style randomized order (cyclic-group permutation —
  stateless, O(1) memory, no repeats) up to `max_random_hosts` (default 10,000,
  auto-raised to the full public space when unset on a literal full sweep).
...
checked by `cfg_prompt_*` functions after per-request custom_prompts and the
RunContext target, and before interactive stdin.
